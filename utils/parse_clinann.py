...
"""
1. Increased Function: Function greater than normal function.
2. Normal Function: Fully functional.
3. Decreased Function: Function less than normal function but greater than no function.
4. No Function: Non-functional or expected non-functional based on allele definition (e.g. early stop codon, frameshift, complete gene deletion).
5. Unkown Function: No literature describing function, or the allele is novel.
6. Uncertain Function: Literature supporting function is insufficient, conflicting, or weak.
"""


def infer_response(text, category):
  ...
  # No function
    ...
    function = "No"
  # Uncertain function
  elif re.search("^The current evidence base suggests (?:that there|there that) is (?:no|not) (?:significant )?association|^No significant association|^This genotype has no significant effect on|Current literature evidence finds no significant effect of|did not have a statistically significant different|The association between the .. genotype and .* is currently unclear|^It is currently unclear|It is not clear*|Patients with .* may (?:not )?respond to(?!.*increased|decreased)|undocumented response|^No significant (?:results|findings)|uncertain significance", text, re.IGNORECASE):
    function = "Uncertain"
  # Unknown function
  ...
  ### Toxicity
    ...
    high_regL = ["greater QT-interval shortening effect", "later onset of bortezomib-induced peripheral neuropathy", "greater elevations of LDL", "higher drug toxicity", "greater prolactin response", \
    ...
    low_regL = ["smaller QT-interval shortening effect", "decreased function by CPIC", "earlier onset of bortezomib-induced peripheral neuropathy", "smaller elevations of LDL", "lower drug toxicity", "lower odds", \
  ...
  ### Metabolism/PK, Other
  elif category == "Metabolism/PK" or category == "Other":
