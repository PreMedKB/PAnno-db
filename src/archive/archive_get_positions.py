...
pos_without_rsid = []
for g in gene_list:
  gene_definition_file = "./definition/%s_allele_definition_table.txt" % (g)
...
pos_rsid_df.to_csv('./assets/cpat.bed', sep='\t', header=0, index=0)



import pandas as pd
cpat_hla_bed = pd.read_csv('./assets/cpat.hla.bed', sep='\t')
chr = cpat_hla_bed[cpat_hla_bed.chrom.str.contains('^chr', regex=True)]
chr.chromStart = chr.chromStart - 5
chr.chromEnd = chr.chromEnd + 5
hla = cpat_hla_bed[cpat_hla_bed.chrom.str.contains('^HLA', regex=True)]
cpat_hla_padding_bed = pd.concat([chr, hla])
cpat_hla_padding_bed.to_csv('./assets/cpat.hla.padding.bed', sep='\t', index=0)


########## 更新：将cpat_diplotype相关的位点加入到bed文件中
cpat_hla_bed = pd.read_csv('./assets/bed/cpat.hla.bed', sep='\t', header=None)
...
merged_df = pd.concat([df, cpat_hla_bed]).iloc[:,0:3]
...
final_bed.to_csv('./assets/bed/cpat.hla.padding20.bed', sep='\t', index=0, header=0)