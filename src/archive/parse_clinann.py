...
def process_uncomplete(uncomplete):
  ...
  for index, row in uncomplete.iterrows():
    ...
    ### Add the function
    row["PAnno Function"] = function
    uncomplete.iloc[index] = row
  
  need_manual = uncomplete[uncomplete["PAnno Function"].isna()]
  rule_res = uncomplete[uncomplete["PAnno Function"].isna() == False]
